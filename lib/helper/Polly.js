...
class Polly extends Helper {
  constructor(config) {
    ...
    adapter = requireg('@pollyjs/adapter-puppeteer');
  }

  static _checkRequirements() {
    try {
      requireg('@pollyjs/core');
      requireg('@pollyjs/adapter-puppeteer');
    } catch (e) {
      return ['@pollyjs/core@^2.5.0', '@pollyjs/adapter-puppeteer@^2.5.0'];
    }
  }
  ...
  async _before() {
    recorder.retry({
      retries: 3,
      when: err => err.message.indexOf('UnhandledPromiseRejectionWarning') > -1,
    });
  }
